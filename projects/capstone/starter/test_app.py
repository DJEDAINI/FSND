...
class CapstoneTestCase(unittest.TestCase):
    ...
    def test_patch_movie(self):
        ...
        res = self.client().put(
            "/api/v1/movies/{}".format(random.choice(movies['movies'])['id']),
            json={
                'title': 'Movie title updated - unittest',
    ...
    def test_patch_actor(self):
        ...
        res = self.client().put(
            "/api/v1/actors/{}".format(random.choice(actors['actors'])['id']),
            json={
                'name': 'Actor name updated - unittest',
                'age': 30,
                'gender': 'female'
            },
            headers=self.headers
    ...
    def test_422_patch_actor(self):
        ...
        res = self.client().put(
            "/api/v1/actors",
            json={
                'title': 'Actor name updated - unittest'
            },
            headers=self.headers
        )
        data = json.loads(res.data)
    ...
    def test_create_movie_for_casting_director(self):
        ...
        res = self.client().post(
            "/api/v1/movies",
            json={
                'title': 'Movie title - unittest', 'release_date': '12/12/2020'
            },
            headers={
                'Authorization': os.environ.get('DIRECTOR')
            }
    ...
    def test_delete_movie_for_executive_producer(self):
        ...
        movie = self.client().post(
            "/api/v1/movies",
            json={
                'title': 'Movie title - unittest executive producer',
                'release_date': '12/12/2020'
            },
            headers=self.headers
        )
        ...
        res = self.client().delete("/api/v1/movies/{}".format(
            random.choice(movies['movies'])['id']), headers=self.headers
        )
        data = json.loads(res.data)
        self.assertEqual(res.status_code, 200)
        self.assertEqual(data['status'], True)
